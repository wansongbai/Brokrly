...
using Dobany.Migrations.Seed.Host;
...
namespace Dobany.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : DobanyAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<DobanyDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{DobanyConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{DobanyConsts.ConnectionStringName}", constring);
        }
