...
using Dobany.Authorization.Accounts.Dto;
...
using Dobany.Web.Models.TokenAuth;
using Dobany.Authorization.Impersonation;
using Dobany.Authorization.Roles;
using Dobany.Configuration;
using Dobany.Identity;
using Dobany.Net.Sms;
using Dobany.Notifications;
using Dobany.Security.Recaptcha;
using Dobany.Web.Authentication.External;
using Dobany.Web.Common;
using Dobany.Authorization.Delegation;

namespace Dobany.Web.Controllers
{
    [Route("api/[controller]/[action]")]
    public class TokenAuthController : DobanyControllerBase
    {
        ...
        /// <returns></returns>
        private async Task<AbpLoginResult<Tenant, User>> GetLoginResultByPhoneCaptchaAsync(string phoneNumber, string captcha, string tenancyName, string nickName)
        {
            var loginResult = await _logInManager.LoginByMobileAsync(phoneNumber, captcha, tenancyName,false, nickName);
            switch (loginResult.Result)
            {
                case AbpLoginResultType.Success:
                    return loginResult;
