            ...
            List<ImportUserDto> invalidUsers)
        {
            ...
            else
            {
                await _appNotifier.SendMessageAsync(
                    args.User,
                    new LocalizableString("AllUsersSuccessfullyImportedFromExcel", DobanyConsts.LocalizationSourceName),
                    null,
            ...
            using (var uow = _unitOfWorkManager.Begin())
            {
                using (CurrentUnitOfWork.SetTenantId(args.TenantId))
                {
                    AsyncHelper.RunSync(() => _appNotifier.SendMessageAsync(
                        args.User,
                        new LocalizableString("FileCantBeConvertedToUserList", DobanyConsts.LocalizationSourceName),
                        null,
