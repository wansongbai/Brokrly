...
{
    public class UserRegistrationManager : DobanyDomainServiceBase
    {
        ...
        public async Task<User> RegisterAsync(string name, string surname, string emailAddress, string userName, string plainPassword, bool isEmailConfirmed, string emailActivationLink, string nickName = "")
        {
            CheckForTenant();
            ...
            var isNewRegisteredUserActiveByDefault = true;
                //await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.IsNewRegisteredUserActiveByDefault);

            await _userPolicy.CheckMaxUserCountAsync(tenant.Id);

            //2021-3-14 09:33:50  增加昵称
            var user = new User
            {
                TenantId = tenant.Id,
                Name = name,
